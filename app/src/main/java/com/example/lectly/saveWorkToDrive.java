...
public class saveWorkToDrive extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_save_work_to_drive);
 ...
 /*   private void showAuthOptions() {
    ...
    public void startGoogleWithWebView() {
        WebView webViewGoogleDrive = (WebView) findViewById(R.id.workWebViewGoogleDrive);
        ...
        try {
            ...
            gdAuthManager.startGoogleDriveAuth(saveWorkToDrive.this, webViewGoogleDrive, this.gdAuthConfig, new GDAuthManager.OnGoogleAuthCompleteListener() {
                @Override
 ...
 /*   private void startGoogleSignInAuth() {
                ...
                public void onSuccess(final GDAuthResponse gdAuthResponse) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            boolean isAuthDataSaved = GDAuthManager.getInstance().setAuthData(saveWorkToDrive.this, gdAuthResponse);
    ...
    public void testGoogleDriveUpload(final GDAuthResponse gdAuthResponse) {
        File tempFile = GDFileManager.getInstance().createTempFile(createWorkFile.workfileNameToSave, getApplicationContext(), "pdf", false);
        try {
            // Create a temp Text file
            GDFileManager.getInstance().saveStringToFile(tempFile, createWorkFile.workToSave);

            // Upload this file to google drive.
            GDApiManager.getInstance().uploadFileAsync(getApplicationContext(), gdAuthResponse, saveWorkToDrive.this.gdAuthConfig, tempFile, GDFileManager.getInstance().getMimeType(getApplicationContext(), tempFile), false, new GDUploadFileResponse.OnUploadFileCompleteListener() {
                @Override
                public void onSuccess(GDUploadFileResponse uploadFileResponse) {
                    showToast("File Uploaded Successfully");

                    // Download just uploaded file
                    GDApiManager.getInstance().downloadFileAsync(getApplicationContext(), gdAuthResponse, saveWorkToDrive.this.gdAuthConfig, uploadFileResponse.getId(), "downloaded_file.txt", new GDDownloadFileResponse.OnDownloadFileCompleteListener() {
                        @Override
    ...
    private void showToast(final String message) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(saveWorkToDrive.this, message, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void showAlert(final String text) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                new AlertDialog.Builder(saveWorkToDrive.this)
